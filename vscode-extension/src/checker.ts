// Static checker for TakoScore v2.0 source (VS Code version - no file system access)

import type { Score, Statement, Expression, ProcDeclaration, GlobalStatement, PartDeclaration, PhraseBlock } from './types/ast';
...
export class Checker {
  ...
  check(score: Score, filePath: string = 'score.mf'): Diagnostic[] {
    this.diagnostics = [];
  ...
  private checkGlobalStatement(stmt: GlobalStatement, filePath: string): void {
    switch (stmt.kind) {
      case 'ImportStatement':
        // VS Code version: just register symbols, no file access
        break;
    ...
    switch (stmt.kind) {
      ...
      case 'ForStatement':
        ...
        if (!this.isConstant(stmt.range.start) || !this.isConstant(stmt.range.end)) {
          this.addWarning('W401', 'For range bounds are not compile-time constants', stmt.position, filePath);
        }
  ...
  private checkRecursion(): void {
    ...
    const dfs = (proc: string, path: string[]): boolean => {
      if (stack.has(proc)) {
        this.addWarning('W310', `Recursion detected: ${[...path, proc].join(' -> ')}`);
        return true;
      }
