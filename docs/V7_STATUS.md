...
このドキュメントは、[docs/V4_SPEC.md](V4_SPEC.md) / [docs/V4_RATIONALE.md](V4_RATIONALE.md) に書かれている言語機能について、現行実装（`src/` の compiler）の状況を棚卸ししたものです。

**最終更新: 2026-01-18**

**バージョン情報:**
- **言語バージョン: v7.0** (TakoMusic language version)
- **IR バージョン: v4** (Score IR schema version - backward compatible)
- **プロファイルバージョン: v1** (Render profile schema)
...
**注:** V4_SPEC.md/V4_RATIONALE.md は言語機能の設計ドキュメントです。言語バージョンは v7 ですが、これらの機能仕様ドキュメント名は歴史的な理由で V4 のまま維持されています。
...
| humanize() 戻り値 | タプル `(Rng, Clip)` | 配列 `[rng, clip]` | v7.0では配列のまま、v8.0でタプル化検討 |
...
| 動的PosRef範囲 | `place 1:1..8:1 repeat(...)` | 未実装 | v7.x/v8.0以降で検討 |
...
| std:midi | モジュール全体 | 未実装（v7.x/v8.0以降） |
| std:audio | モジュール全体 | 未実装（v7.x/v8.0以降） |
...
- [x] V7* エクスポートを追加（V7Lexer, V7Parser, V7Compiler, V7Evaluator）
- [x] CLI ヘルプを更新
- [x] package.json version を 7.0.0 に更新
- [x] 標準ライブラリコメントを v7.0 に更新
- [x] CHANGELOG.md に v7.0 の変更点を記載
- [x] vscode-extension のバージョンを同期 (7.0.0)
- [x] examples/*.mf, domo/*.mf の旧バージョン参照を削除
- [x] docs/BUILTINS.md, STDLIB.md, RENDERING.md の旧バージョン参照を削除
...
1. ~~**v7.0 リリース**: 主要機能はすべて実装済み~~ → ✅ **リリース完了**
   - 212テスト全てパス
   - Web Playground リアルコンパイル対応
   - LSP 定義ジャンプ・参照・シグネチャヘルプ対応
   - CHANGELOG.md 更新済み
   - 全stdlib v7.0 に統一
2. **v7.x/v8.0 以降**: struct, Result型, 動的PosRef, std:midi, std:audio などの追加機能
